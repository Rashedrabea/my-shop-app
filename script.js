...
function updateAdminPanel() {
    ...
    users.forEach(uid => {
        ...
        if (user && user.lastLogin && new Date(user.lastLogin) > fiveMinutesAgo) onlineCount++;
    });
    ...
    users.forEach(uid => {
        ...
        tableHtml += `<tr style="border-bottom:1px solid #eee;">
            ...
            <td style="padding:8px;"><span style="background:${isOnline ? '#00B894' : '#999'};color:white;padding:2px 10px;border-radius:12px;font-size:11px;">${isOnline ? '🟢 متصل' : '🔴 غير متصل'}</span></td>
        </tr>`;
    });
...
function updateUI() {
    ...
    document.getElementById('saleList').innerHTML = (data.sales || []).slice().reverse().map(s => `
        <div class="list-item"><span>${s.client || ''} - ${s.product || ''}</span><div><span style="font-weight:bold;">${s.total || 0} ج.م</span></div></div>
    ...
    document.getElementById('purchaseList').innerHTML = (data.purchases || []).slice().reverse().map(p => `
        <div class="list-item"><span>${p.supplier || ''} - ${p.product || ''}</span><div><span style="font-weight:bold;">${p.total || 0} ج.م</span></div></div>
...
function searchByBarcode() {
    ...
    if (product) { document.getElementById('saleProduct').value = product.id; showToast('✅ تم العثور على: ' + product.name); }
...
function addSale() {
    ...
    if (!client || !productId || qty <= 0) return showToast('⚠️ أكمل البيانات');
    if (price <= 0) return showToast('⚠️ السعر أكبر من 0');
...
function addPurchase() {
    ...
    if (!supplier || !productId || qty <= 0) return showToast('⚠️ أكمل البيانات');
    if (price <= 0) return showToast('⚠️ السعر أكبر من 0');
    const prod = data.products.find(p => p.id === productId);
...
function paySupplier() {
    ...
    if (data.treasury < amount) return showToast('⚠️ الرصيد غير كافي');
    data.treasury -= amount;
...
function drawCharts() {
    ...
    chartInstances.sales = new Chart(document.getElementById('salesChart'), {
        ...
        data: {
            ...
            datasets: [{ label: 'الحركة المالية (ج.م)', data: [salesTotal, purchasesTotal, expensesTotal, collectionsTotal], backgroundColor: ['#2E4057', '#E17055', '#FDCB6E', '#00B894'], borderColor: ['#1a2a3a', '#c0392b', '#f39c12', '#00a87a'], borderWidth: 2, borderRadius: 8 }]
        },
    ...
    chartInstances.treasury = new Chart(document.getElementById('treasuryChart'), {
        ...
        data: {
            ...
            datasets: [{ data: [treasury, expensesTotal, purchasesTotal], backgroundColor: ['#2E4057', '#E17055', '#FDCB6E'], borderWidth: 3, borderColor: '#fff' }]
        },
        options: {
            responsive: true, maintainAspectRatio: false,
            plugins: {
                legend: { position: 'bottom', labels: { padding: 15, usePointStyle: true, pointStyle: 'circle' } },
                tooltip: { callbacks: { label: function(context) { const total = context.dataset.data.reduce((a, b) => a + b, 0); const percentage = total > 0 ? ((context.parsed / total) * 100).toFixed(1) : 0; return context.label + ': ' + context.parsed + ' ج.م (' + percentage + '%)'; } } }
            }
        }
    });
}
...
function showReport(type) {
    ...
    if (type === 'income') {
        ...
        if (!clientName) {
            ...
            clientCollections.forEach(c => { balance -= (c.amount || 0); rows += `<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #333;font-size:13px;"><span style="color:#aaa;">${new Date(c.id).toLocaleDateString()}</span><span>تحصيل</span><span style="color:#00B894;">${c.amount || 0} ج.م</span><span>${balance} ج.م</span></div>`; });
            html = `<div style="background:#252538;border-radius:12px;padding:15px;color:white;">
...
function printAllInvoices() {
    ...
    sales.forEach(s => {
        printContent += `<div style="border:1px solid #ddd;padding:10px;margin:10px 0;border-radius:8px;">
            <p><strong>الفاتورة:</strong> ${s.id || ''} | <strong>العميل:</strong> ${s.client || ''} | <strong>المنتج:</strong> ${s.product || ''}</p>
            <p><strong>الكمية:</strong> ${s.qty || 0} | <strong>السعر:</strong> ${s.price || 0} | <strong>الإجمالي:</strong> ${s.total || 0} ج.م</p>
            <p><strong>التاريخ:</strong> ${s.date || ''}</p>
        </div>`;
    });
    printContent += '</div>';
    const win = window.open('', '_blank');
    if (win) { win.document.write('<html><head><meta charset="UTF-8"><title>طباعة الفواتير</title><style>body{font-family:Arial;padding:20px;}</style></head><body>' + printContent + '</body></html>'); win.document.close(); win.print(); }
}
...
console.log('✅ نظام راشد V31 - النسخة النهائية');
